...
const PackingMethod = ({ searchQuery, isModalOpen, onClose }) => {
  const [data, setData] = useState([]);
  const [editedPacking, setEditedPacking] = useState("");
  ...
  const handleStatusToggle = async ({ id, isActive }) => {
    try {
      ...
      console.error(
        `Error toggling status for packing Methods with ID ${id}:`,
        error
      );
  ...
  const handleEditClick = ({ id, packingType }) => {
    setEditIndex(id);
    setEditedPacking(packingType);
  };

  // handle input change
  const handleInputChange = (e) => {
    setEditedPacking(e.target.value);
  };
  ...
  const handleSaveClick = async (index, id) => {
    try {
      ...
      console.error(`Error saving packing Type with ID ${id}:`, error);
      // Handle error as needed
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error("Error deleting packing Methods:", error);
      // Handle error as needed
    }
  };
  ...
  const handleSinglePacking = async () => {
    try {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50 w-full">
            <tr>
              ...
              <th className="px-2 py-3 text-left text-md font-bold text-black uppercase 2xl:w-[500px] xl:w-[450px] min-w-[200px]">
                Packing Method
              </th>
            ...
            <div className="py-2 flex flex-col">
              ...
              <div className="flex flex-col items-center">
                {/* <p className="text-gray-400 font-bold mt-10">
                  *For multiple “Packing method ” feed use enter after each
                  values
                </p> */}
                <input
